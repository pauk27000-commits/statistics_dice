...
export class StatisticsUI extends Application {
    ...
    async _onShareExtended(event) {
        ...
        const rows = stats.dice.length
            ? stats.dice.map((die, index) => `<tr style="background:${index % 2 === 0 ? 'rgba(255,255,255,0.03)' : 'transparent'};"><td style="padding:6px 8px; font-weight:700; color:#ffcf7d;">${die.type}</td><td style="padding:6px 8px;">${die.rolls}</td><td style="padding:6px 8px;">${die.average}</td><td style="padding:6px 8px;" class="${die.luckCssClass}">${die.luckTier}</td><td style="padding:6px 8px;">${die.explosions}</td></tr>`).join('')
    ...
    async _onShareSkills(event) {
        ...
        const rows = stats.skillUsage.length
            ? stats.skillUsage.map((skill, index) => `<tr style="background:${index % 2 === 0 ? 'rgba(255,255,255,0.03)' : 'transparent'};"><td style="padding:6px 8px; text-align:left;">${skill.name}</td><td style="padding:6px 8px; color:#ffcf7d; font-weight:700;">${skill.count}</td><td style="padding:6px 8px; color:#a7f3b0;">${skill.max}</td></tr>`).join('')
